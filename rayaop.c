...
char* rayaop_generate_intercept_key(zend_string *class_name, zend_string *method_name, size_t *key_len) {
    char *key = NULL;
    *key_len = spprintf(&key, 0, "%s::%s", ZSTR_VAL(class_name), ZSTR_VAL(method_name));  // Generate key in the format class_name::method_name
...
intercept_info* rayaop_find_intercept_info(const char *key, size_t key_len) {
    return zend_hash_str_find_ptr(RAYAOP_G(intercept_ht), key, key_len);  // Search for intercept information in hash table
}
...
void rayaop_execute_intercept(zend_execute_data *execute_data, intercept_info *info) {
    if (Z_TYPE(info->handler) != IS_OBJECT) {  // If handler is not an object
        return;  // Abort processing
    }

    if (execute_data->This.value.obj == NULL) {  // If target object is NULL
    ...
    for (uint32_t i = 0; i < arg_count; i++) {  // For each argument
        zval *arg = &args[i];
    ...
    if (call_user_function(NULL, &info->handler, &func_name, &retval, 3, params) == SUCCESS) {  // Call intercept function
        if (!Z_ISUNDEF(retval)) {  // If return value is defined
            ZVAL_COPY(execute_data->return_value, &retval);  // Copy return value
        }
    } else {  // If interception fails
        php_error_docref(NULL, E_WARNING, "Interception failed for %s::%s", ZSTR_VAL(info->class_name), ZSTR_VAL(info->method_name));  // Output warning
    }
...
void rayaop_free_intercept_info(zval *zv) {
    ...
    if (info) {  // If intercept information exists
        RAYAOP_DEBUG_PRINT("Freeing intercept info for %s::%s", ZSTR_VAL(info->class_name), ZSTR_VAL(info->method_name));  // Output debug information
...
static void rayaop_zend_execute_ex(zend_execute_data *execute_data) {
    ...
    if (!rayaop_should_intercept(execute_data)) {  // If interception is not necessary
    ...
    if (info) {  // If intercept information is found
    ...
    } else {  // If intercept information is not found
...
PHP_FUNCTION(method_intercept) {
    ...
    if (!new_info) {  // If memory allocation fails
    ...
    if (zend_hash_str_update_ptr(RAYAOP_G(intercept_ht), key, key_len, new_info) == NULL) {  // Add to hash table
...
static const zend_function_entry ray_aop_method_interceptor_interface_methods[] = {
    ZEND_ABSTRACT_ME(Ray_Aop_MethodInterceptorInterface, intercept, arginfo_ray_aop_method_interceptor_intercept)  // Define intercept method
...
PHP_MINIT_FUNCTION(rayaop)
{
    ...
    zend_class_entry ce;
    INIT_CLASS_ENTRY(ce, "Ray\\Aop\\MethodInterceptorInterface", ray_aop_method_interceptor_interface_methods);  // Initialize class entry
...
PHP_RINIT_FUNCTION(rayaop)
{
    ...
    if (RAYAOP_G(intercept_ht) == NULL) {  // If intercept hash table is not initialized
